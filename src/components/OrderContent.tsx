    ...
    return (
        <>
            ...
            <div className="border-2 border-neutral-300 p-5 rounded-sm">
                {order.length === 0 ? 'No hay ninguna orden'
                : (
                    order.map(item => (
                        <div
                            key={item.id}
                        ...
                        >
                            <div>
                                ...
                                <p className="text-neutral-700">{formatCurrency(item.quantity)}</p>
                                <p className="text-neutral-700 font-medium">Cantidad: {item.quantity}</p>
                                <p className="text-neutral-700 font-bold">Total: {formatCurrency(item.quantity * item.quantity)}</p>
                            </div>
                            <button
